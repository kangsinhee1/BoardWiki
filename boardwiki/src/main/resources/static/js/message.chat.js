$(function(){
	...
	//채팅방 또는 채팅페이지를 인식해서 채팅방 멤버를 초기 셋팅
	 ...
	 $.ajax({
		...
		success:function(param){
			if(param.result =='logout'){
				$('#member_search').attr('disabled',ture);
				$('#member_search').val('로그인 해야 회원검색이 가능합니다.');
			}else if(param.result == 'success'){
				$('#search_area').empty();
				$(param.member).each(function(index,item){
					//채팅방 개설자의 아이디와 동일한 아이디를 체크 또는 이미 member_list에 저장된 아이디 체크 후제외

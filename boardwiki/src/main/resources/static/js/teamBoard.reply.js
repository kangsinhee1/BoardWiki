$(function(){
	...
	function selectList(pageNum){
		...
		$.ajax({
			url:'listBoardReply',
			type:'get',
			data:{teaB_num:$('#teaB_num').val(),pageNum:pageNum,rowCount:rowCount},
			dataType:'json',
			beforeSend:function(){
			...
			success:function(param){
				...
				$(param.list).each(function(index,item){
					...
					if(item.re_mdate){
						...
						output += '<span class="modify-date">등록일 : ' + item.teaR_rdate + '</span>';
					}
					output +='        </li>';
					output +='     </ul>';
					output +='     <div class="sub-item">';
					output += '    <p>' + item.teaR_content.replace(/\r\n/g,'<br>') + '</p>';
					
					if(param.user_num===item.mem_num){
						//로그인 한 회원번호와 댓글 작성자 회원번호가 같으면
						output += '  <input type="button" data-num="'+item.teaR_num+'" value="수정" class="modify-btn">';
						output += '  <input type="button" data-num="'+item.teaR_num+'" value="삭제" class="delete-btn">';
					}
						output += '  </div>';
					output += '</div>';
	...
	$('#re_form').submit(function(event){
		if($('#teaR_content').val().trim()==''){
			alert('내용을 입력하세요');
			$('#teaR_content').val('').focus();
			return false;
		}
	...
	$(document).on('click','.modify-btn',function(){
		...
		//댓글 내용
		let teaR_content = $(this).parent().find('p').html().replace(/<br>/gi,'\r\n');
		...
		//입력한 글자수 셋팅
		let inputLength = $('#mteaR_content').val().length;
	...
	$(document).on('submit','#mre_form',function(event){
		if($('#mteaR_content').val().trim()==''){
			alert('내용을 입력하세요!');
			$('#mteaR_content').val('').focus();
			return false;
		}
		...
		$.ajax({
			...
			success:function(param){
				if(param.result=='logout'){
					...
					//수정 데이터 표시
					$('#mre_form').parent().find('p').html($('#mteaR_content').val().replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\r\n/g,'<br>').replace(/\r/g,'<br>').replace(/\n/g,'<br>'));
					//최근 수정일 처리
	...
	$(document).on('keyup','textarea',function(){
		...
		if(inputLength>300){//300자를 넘어선 경우
			...
			let remain = 300 - inputLength;
			remain += '/300';
			if($(this).attr('id')=='teaR_content'){
				//댓글 등록 폼 글자수
				$('#re_first .letter-count').text(remain);
			}else if($(this).attr('id')=='mteaR_content'){
				//댓글 수정 폼 글자수
				$('#mre_first .letter-count').text(remain);
	...
	$(document).on('click','.delete-btn',function(){
		//댓글 번호
		let teaR_num = $(this).attr('data-num');
		//서버와 통신
		$.ajax({
			url:'deleteReply',
			type:'post',
			data:{teaR_num:teaR_num},
			dataType:'json',
			success:function(param){
				if(param.result == 'logout'){

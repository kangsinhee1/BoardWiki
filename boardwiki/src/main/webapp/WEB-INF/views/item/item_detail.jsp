	...
	<div class="container">
		...
		<div class="row">
			<div class="col-xl-9 col-lg-8 col-md-7 game-single-content">
				<div class="gs-meta">
					boardgameGeek / in <a
						href="https://boardgamegeek.com/browse/boardgame">BGG</a>
				</div>
				...
				<div class="geme-social-share pt-5 d-flex">
					<p>
						<a href="/item/item_detail?item_num=${item.item_num }">간단설명</a> <a
							href="/rent/rent?item_num=${item.item_num}">대여</a> <a
							href="/used/usedList?item_num=${item.item_num }">중고거래</a> <a
							href="/tnrboard/tnrboardList?tnr_category=1&item_num=${item.item_num }">팁</a>
						<a
							href="/tnrboard/tnrboardList?tnr_category=2&item_num=${item.item_num }">후기</a>
				...
				<div id="stickySidebar">
					<div class="widget-item">
						<div class="rating-widget">
							...
							<div class="rating">
								<h5>
									<i>평점</i><span>${item.item_average}</span> / 10
								</h5>
							</div>
						</div>
					</div>
								...
								<c:if test="${!empty member.mem_num}">
									<div class="text-box2"
										...
										<form id="addToCart" method="get"
											action="${pageContext.request.contextPath}/cart/cart">
											<input type="hidden" name="item_num" value="${item.item_num}" />
											<input type="hidden" name="user" value="${member.mem_num}" />

											<div class="cart_box">
												<label class="quantity_label" for="quantity">수량</label> <input
													type="number" id="quantity" name="item_quantity" value="1"
													min="1" max="${item.item_stock}" />
												<c:if test="${item.item_stock > 0}">
                                                <button type="button" class="addToCartButton" id="addToCartButton">장바구니에 담기</button>
												</c:if>
											</div>
										</form>
									</div>

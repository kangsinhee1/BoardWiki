			...
			<div class="col-lg-12">
				...
				<div class="page-main">
					...
					<form action="rentListAdmin" id="search_form" method="get">
						<ul class="rent_search">
							<li><select name="keyfield" id="keyfield">
									...
									<option value="3"
										<c:if test="${param.keyfield == '3'}">selected</c:if>>닉네임</option>
							</select></li>
						...
						<div class="align-right">
							<select id="order" name="order">
								...
								<option value="2"
									<c:if test="${param.order == '2'}">selected</c:if>>번호순</option>
								<option value="3"
					...
					<script type="text/javascript">
						...
						document.getElementById('keyfield').addEventListener(
								'change',
								function() {
									var keyfield = this.value;
									var dateFields = document
											.getElementById('dateFields');
									if (keyfield == '2') {
									...
									<c:forEach var="rent" items="${list}">
										<tbody>
											<tr>
												...
												<td class="align-center td-name"
													style="border: none !important;">${rent.item_name}</td>
												...
												<td class="align-center">${rent.mem_email}</td>
												<!-- 회원 Email 표시 -->
												<td class="align-center">${rent.mem_nickname}</td>
												<!-- 회원 Nickname 표시 -->
												<td class="align-center"><c:choose>
														<c:when test="${rent.rent_status == 1}">
															<a
																...
																onclick="return confirm('반납 완료 처리하시겠습니까?');">대여중</a>
														</c:when>
